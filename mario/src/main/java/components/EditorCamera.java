...
public class EditorCamera extends  Component{
    ...
    public void update(float dt){
        if (MouseListener.mouseButtonDown(GLFW_MOUSE_BUTTON_LEFT) && dragDebounce > 0){
            this.clickOrigin = new Vector2f(MouseListener.getOrthorX(), MouseListener.getOrthorY());
            dragDebounce -= dt;
            return;
        } else if (MouseListener.mouseButtonDown(GLFW_MOUSE_BUTTON_LEFT)){
            Vector2f mousePos = new Vector2f(MouseListener.getOrthorX(), MouseListener.getOrthorY());
